...
class DistritoController extends Controller
{
    ...
    public function Index(){
        ...
        return \view('dist/distrito/index');
    }
...
//return 
		...
		$query = DB::table('ca_distrito')
        ->leftjoin('ca_pais', 'ca_pais.id', '=', 'ca_distrito.paisId')
        ->leftjoin('ca_pais', 'ca_pais.id', '=', 'ca_distrito.paisId')
		 ->select('ca_distrito.*', 'ca_pais.descripcion as pais')
        ...
        if(isset($request['searchInput']) && trim($request['searchInput']) != ""){
			$query->where(
				function ($query) use ($request) {
					$query->orWhere('ca_distrito.descripcion', 'like', '%'.trim($request['searchInput']).'%');
					$query->orWhere('ca_distrito.codigo', 'like', '%'.trim($request['searchInput']).'%');
				}
			 );		
		}
		...
		foreach($result['data'] as $value){
			...
			if($value->estatus == 'Activo'){
				$detalle = '<a href="/dist/distrito/mostrar/'.$value->id.'" class="btn btn-icon waves-effect waves-light bg-warning text-white m-b-5"> <i class="bi bi-eye"></i> </a>
								<a href="/dist/distrito/editar/'.$value->id.'" class="btn btn-icon waves-effect waves-light bg-secondary text-white m-b-5"> <i class="bi bi-pencil"></i> </a>
