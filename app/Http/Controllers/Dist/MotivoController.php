...
class MotivoController extends Controller
{
    ...
    public function PostIndex(){
        ...
        $query = DB::table('motivo')
        ->leftjoin('departamento', 'departamento.id', '=', 'motivo.departamentoId')
         ->select('motivo.*', 'departamento.nombre as departamento')
        ...
        if(isset($request['searchInput']) && trim($request['searchInput']) != ""){
            $query->where(
                function ($query) use ($request) {
                    ...
                    $query->orWhere('motivo.codigo', 'like', '%'.trim($request['searchInput']).'%');
    ...
    public function Nuevo(){
        ...
        return \View::make('dist/motivo/nuevo');
    }
    ...
    public function postNuevo(){
        ...
        $motivoExiste = Motivo::where('descripcion', $this->request->nombre)
        //->where('distribuidorId', Auth::user()->distribuidorId)
        ->first();
        if(!empty($motivoExiste)){
            return redirect('dist/motivo/nuevo')->withErrors("ERROR AL GUARDAR STORE CEBECECO CODE-0001");
        }
    
        DB::beginTransaction();
        try { 	
            $motivo = new Motivo;
            $motivo->descripcion         = trim($this->request->nombre);
    
            if(isset($this->request->comentario)){
                $motivo->infoextra       = trim($this->request->comentario); 
            }
            
            $motivo->estatus          = 'Activo';
            //$motivo->prioridad        = trim($this->request->prioridad); 
            $motivo->created_at       = date('Y-m-d H:i:s');
            $motivo->usuarioId        = Auth::user()->id;
            $motivo->departamentoId   = trim($this->request->departamento);
            $result = $motivo->save();
            ...
            if(empty($motivoId)){
                DB::rollBack();
                return redirect('dist/motivo/nuevo')->withErrors("ERROR AL GUARDAR EL CONTRATO NO SE GENERO UN # DE CONTRATO CORRECTO CODE-0196");
            }
            
            $motivoCode = str_pad($motivoId,5, "0",STR_PAD_LEFT);
    
            $motivoUpdate = Motivo::find($motivoId);
            $motivoUpdate->codigo = $motivoCode;
            $result = $motivoUpdate->save();	
    
        } catch(\Illuminate\Database\QueryException $ex){ 
            DB::rollBack();
            return redirect('dist/motivo/nuevo')->withErrors('ERROR AL GUARDAR STORE CEBECECO CODE-0002'.$ex);
        }
        
        if($result != 1){
            DB::rollBack();
            return redirect('dist/motivo/nuevo')->withErrors("ERROR AL GUARDAR STORE CEBECECO CODE-0003");
        }
        DB::commit();
    
        return redirect('dist/motivo')->with('alertSuccess', 'STORE CEBECECO HA SIDO INGRESADA');
    }
    
    public function Editar($motivoId){
        /*if(!$this->common->usuariopermiso('004')){
            return redirect('dist/dashboard')->withErrors($this->common->message);
        }*/
    ...
    public function PostEditar(){
        ...
        $motivo = Motivo::where('id', $motivoId)
        //->where('distribuidorId',Auth::user()->distribuidorId)
        ->first();
