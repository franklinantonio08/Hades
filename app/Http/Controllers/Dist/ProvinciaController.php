...
class ProvinciaController extends Controller
{
    ...
    public function Index(){
        ...
        return \view('dist/provincia/index');
    }
...
//return 
		...
		$query = DB::table('ca_provincia')
        ->leftjoin('ca_pais', 'ca_pais.id', '=', 'ca_provincia.paisId')
		 ->select('ca_provincia.*', 'ca_pais.descripcion as pais')
        ...
        if(isset($request['searchInput']) && trim($request['searchInput']) != ""){
			$query->where(
				function ($query) use ($request) {
					$query->orWhere('ca_provincia.descripcion', 'like', '%'.trim($request['searchInput']).'%');
					$query->orWhere('ca_provincia.codigo', 'like', '%'.trim($request['searchInput']).'%');
				}
			 );		
		}
		   
		$provincia = $query->paginate($length); 
		...
		foreach($result['data'] as $value){
			...
			if($value->estatus == 'Activo'){
				$detalle = '<a href="/dist/provincia/mostrar/'.$value->id.'" class="btn btn-icon waves-effect waves-light bg-warning text-white m-b-5"> <i class="bi bi-eye"></i> </a>
								<a href="/dist/provincia/editar/'.$value->id.'" class="btn btn-icon waves-effect waves-light bg-secondary text-white m-b-5"> <i class="bi bi-pencil"></i> </a>
