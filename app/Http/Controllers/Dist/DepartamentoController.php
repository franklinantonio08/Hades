...
class DepartamentoController extends Controller
{
    ...
    public function Index(){
        ...
        return \view('dist/departamento/index');
    }
        ...
        if(isset($request['searchInput']) && trim($request['searchInput']) != ""){
			$query->where(
				function ($query) use ($request) {
					...
					$query->orWhere('departamento.codigo', 'like', '%'.trim($request['searchInput']).'%');
				}
			 );		
		}
		   
		$departamento = $query->paginate($length); 
	
		$result = $departamento->toArray();
		$data = array();
		foreach($result['data'] as $value){

			if($value->estatus == 'Activo'){
				$detalle = '<a href="/dist/departamento/mostrar/'.$value->id.'" class="btn btn-icon waves-effect waves-light bg-warning text-white m-b-5"> <i class="bi bi-eye"></i> </a>
								<a href="/dist/departamento/editar/'.$value->id.'" class="btn btn-icon waves-effect waves-light bg-secondary text-white m-b-5"> <i class="bi bi-pencil"></i> </a>
